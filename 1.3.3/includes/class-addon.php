...
class GFP_Qualpay_Addon extends GFPaymentAddOn {
	...
	public function feed_settings_fields() {
		...
		$feed_field_transaction_type = array(
			...
			//'onchange' => $submit_form_js,
			'onchange'   => "if('authorization'==jQuery('#transaction_type').val()){jQuery('#gform_setting_achOnOff_checkbox').hide(); } else {jQuery('#gform_setting_achOnOff_checkbox').show(); }",
		...
		$feed_field_plan_frequency = array(
			...
			'onchange'   => "if(3==jQuery('#plan_frequency').val()){jQuery('#gform_setting_plan_interval').show(); } else {jQuery('#gform_setting_plan_interval').hide(); }",
			'dependency' => array( 'field' => 'plan_type', 'values' => array( 'one_off' ) )
		);
		...
		$feed_field_ach_on_off = array(
			...
			'hidden'      => ( $this->get_setting( 'transaction_type' ) === 'authorization') ? true : false,
			'choices'    => array(
				array(
					'name'          => 'achOnOff',
					'label'         => '',
		...
		$sections = array(
			...
			'section_ach_css_settings'     => array(
				'title'      => esc_html__( 'ACH and CSS Settings', 'gravityformsqualpay' ),
				'fields'     => array(
	...
	public function gform_field_input( $markup, $field, $value, $lead_id, $form_id ) {
		...
		$tran_type = $feed_meta['payment_type'] == 'one_time' ?? $feed_meta['transaction_type'];
		$achToogle = $feed_meta['achOnOff'];
		$custom_css = $feed_meta['custom_css'];
		if($tran_type != 'authorization') {
			$achVal = '<input type="hidden" id="achOnOff" name="achOnOff" value="' . $achToogle . '" />';
		} else {
			$achVal = '<input type="hidden" id="achOnOff" name="achOnOff" value="0" />';
		}
		$embedded_field = '<div id="qp-embedded-container" style="width:100%" align="center"></div>';
		$embedded_field .= '
		<input type="hidden" id="capture_id" name="capture_id" value="' . $tran_type . '" />'.$achVal;
	...
	public function gform_payment_methods( $payment_methods, $field, $form_id ) {
		...
		if ( is_user_logged_in() && $this->has_feed( $form_id ) ) {
			...
			$card_ids = array();
			if($get_qualpay_card_ids['response']['data']['billing_cards']) {
			...
			if($cards) {
				foreach ( $cards as $card ) {
					$card_id = $card['id'];
					if ( ! empty($mode)  && strpos($card['mode'], $mode) !== false) {
						if (! empty($merchant_id) && strpos($card['merchant_id'], $merchant_id) !== false) {
	...
	public function add_creditcard_token_input( $content, $field, $value, $entry_id, $form_id ) {
		...
		$content .= "<input type='hidden' name='input_{$field->id}.11' id='input_{$form_id}_{$field->id}_11' value='" . rgpost( "input_{$field->id}_11" ) . "' />";
	...
	public function ajax_qualpay_transient_key() {
		...
		$mode = $this->get_qualpay_mode( GFAPI::get_form( $form_id ), true );
	 ...
	 * @return array|string
	 */
	public function get_qualpay_mode( $form, $admin_ajax = false ) {
		// $form_settings = $this->get_form_settings( $form );
		$form_id = $form ? $form['id'] : null;
		$feed_settings = $this->get_feeds( $form_id );
		if( is_admin() && !$admin_ajax) {
			 $feed_set = $feed_settings['meta'];
		} else {
			$feed_set = $feed_settings[0]['meta'];
		}
		return $this->get_setting( 'mode', '', $feed_set );
	}
	...
	public function get_submission_data( $feed, $form, $entry ) {
		...
		switch ( $feed[ 'meta' ][ 'payment_type' ] ) {
			...
			case 'one_time':
				...
				if($this->current_submission_data[ 'card' ] && $this->current_submission_data[ 'card' ][ 'type' ] == 'ACH') {
				...
				$this->current_submission_data[ $transaction_type ] = array(
					...
					'avs_zip'			=> $avs_zip,
					'card_id'          => $this->current_submission_data[ 'card' ] ?? $this->current_submission_data[ 'card' ][ 'id' ],
	...
	private function get_customer() {
		...
		if ( empty( $customer_id ) ) {
			...
			if ( ! empty( $shipping ) ) {
				...
				$shipping[ 'shipping_first_name' ] = $this->current_submission_data[ 'customer_first_name' ] ? $this->current_submission_data[ 'customer_first_name' ] : null;

				$shipping[ 'shipping_last_name' ] = $this->current_submission_data[ 'customer_last_name' ] ? $this->current_submission_data[ 'customer_last_name' ] : null;
			...
			$first_name = $this->current_submission_data[ 'customer_first_name' ] ? $this->current_submission_data[ 'customer_first_name' ] : null;

			$last_name = $this->current_submission_data[ 'customer_last_name' ] ?  $this->current_submission_data[ 'customer_last_name' ] : null;
			...
			if ( ! empty( $billing_form_data ) ) {
				...
				$billing_form_data[ 'billing_first_name' ] =  $this->current_submission_data[ 'customer_first_name' ] ? $this->current_submission_data[ 'customer_first_name' ] : null;

				$billing_form_data[ 'billing_last_name' ] = $this->current_submission_data[ 'customer_last_name' ] ?  $this->current_submission_data[ 'customer_last_name' ] : null;
	...
	private function get_billing_card() {
		...
		if ( empty( $this->current_submission_data[ 'card' ] ) ) {
			...
			if ( $add_new_card ) {
				...
				$new_card_info = GFP_Qualpay_Customer_API::get_billing_card( get_current_user_id(), $current_card_id );
	...
	private function get_info_to_add_new_card() {
		...
		$new_card_info = array(
			...
			'last4'       => substr( rgpost( "input_{$card_field->id}_11" ), - 4, 4 ),
	...
	public function authorize( $feed, $submission_data, $form, $entry ) {
		...
		if ( $transaction_response[ 'success' ] && ! empty( $transaction_response[ 'response' ] ) ) {
			...
			return array(
				...
				'payment_method' => $submission_data[ 'card' ] ?? $submission_data[ 'card' ][ 'type' ]
			);
	...
	public function subscribe( $feed, $submission_data, $form, $entry ) {
		...
		$submission_data[ 'subscription' ] [ 'card_id' ] =  $submission_data[ 'card' ] ?? $submission_data[ 'card' ][ 'id'];
